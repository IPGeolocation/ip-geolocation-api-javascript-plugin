...
The [IPGeolocation API](https://ipgeolocation.io) provides comprehensive geolocation data, including country codes, city names, zip codes, latitude and longitude, EU membership status, calling codes, top-level domains, languages spoken, national flags, currency details, timezone information, security features (VPN, proxy, Tor, etc.), ISP details, connection types and much more. For more detailed Information about the available fields, you can visit our [documentation](https://ipgeolocation.io/ip-location-api.html#documentation-overview).

This SDK allows developers to easily integrate and utilize IP geolocation data within their web projects. Developers can use this SDK for various purposes, such as:
...
2. **Geographic Redirection**: Redirect visitors to region-specific versions of a site to enhance user experience and compliance with local regulations.
...
5. **Form Automation**: Pre-fill forms with country, state, and city information, speeding up the checkout and registration processes.
...
8. **Security Enforcement**: Detect and mitigate malicious activities by identifying traffic from suspicious locations or those using VPNs and proxies.
...
- **ipAddress** (`optional: string`): Specify an IP address to query. If not provided, the client’s IP address is used by default.
...
- **includeHostname** (`optional:  boolean`): Includes hostname lookup from the IP-Hostname database.

- **includeLiveHostname** (`optional:  boolean`): Enables live hostname lookup.

- **includeHostnameFallbackLive** (`optional:  boolean`): Combines hostname lookup from the IP-Hostname database and live sources if no hostname is found.

- **includeSecurity** (`optional:  boolean`): Adds IP-Security information to the response.

- **includeUserAgent** (`optional:  boolean`): Includes information about the client’s user agent.
...
- **ttl** (`optional`): Time-to-live for data stored in local storage, specified in hours (e.g., `3` for 3 hours).

Please note that only one of these three options (`includeHostname`, `includeLiveHostname`, or `includeHostnameFallbackLive`) should be passed at a time. For `caching` the response, you can use either or both storage options, depending on your requirements.
